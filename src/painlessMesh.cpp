...
// Constructor
painlessMesh::painlessMesh(Scheduler *_userScheduler)
{
    if(_userScheduler != NULL)
    {
        _userScheduler->setHighPriorityScheduler(&this->scheduler);
        isExternalScheduler = true;
    }
    else
    {
        isExternalScheduler = false;
    }
}
